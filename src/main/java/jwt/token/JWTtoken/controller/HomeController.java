    ...
    public ResponseEntity<JwtAuthResponse> authenticateUser(@RequestBody JwtAuthRequest jwtAuthRequest){
        ...
        try{
            authenticationManager
                    .authenticate(new UsernamePasswordAuthenticationToken(jwtAuthRequest.getUserName(),
                            jwtAuthRequest.getPassword()));
        } catch (Exception ex){
            throw new UserNameNotFound("Username not found in Database to generate JWT token");
        }
